...
def arrange_L_data(L, delta):
    """
    this function arrange the L list
    :param delta: parmeter that set the bar
    :param L:
...
def maximum_filtering(lst, delta):
    data = arrange_L_data(lst, delta)
    maximum = []
    for lst in data:
...
def maximum_save_order(lst, delta):
    """

    :return:
    """
    maximum = maximum_filtering(lst, delta)
    maxi_in_order = []
    for i in lst:
...
if __name__ == '__main__':
    ...
    print(maximum_filtering(L, Delta))
    print(maximum_save_order(L, Delta))
