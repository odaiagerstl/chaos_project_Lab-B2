...
def saving_data():
    path = r'C:\Google One\University\Third year\Spring\LAB B2\Choas\Diode_carcter\diode carctriztion.csv'
    df = pd.read_csv(path)
...
def Diode_characteristic():
    ...
    mir_CH2_Voltage = - CH2_Voltage
    print(linregress(Current, mir_CH2_Voltage))
    #plt.plot(Current, mir_CH2_Voltage, color='green', linestyle='dashed', linewidth=3,
    #         marker='o', markerfacecolor='blue', markersize=6)
    plt.plot(Current, mir_CH2_Voltage)
    plt.xlabel('Current - axis')
    plt.ylabel('Diode_Voltage - axis')
    plt.title('Diode_Voltage VS Current')
    x = np.linspace(-0.015, 0.010, len(Current))
    y = np.exp(x)
    plt.figure()
    plt.plot(x, y)
    plt.xlabel('$x$')
