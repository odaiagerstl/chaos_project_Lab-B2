...
def local_maximum(input_voltage):
    ...
    peaks, _ = find_peaks(D_Voltage, height=-0.4, distance=29000)
    # print('****** \npeaks is ', peaks)
...
def plot_maximum():
    ...
    :return:
    """
    o_range = list(map(lambda x: x / 10.0, range(18, 52, 2)))
    for i in o_range:
        input_voltage = str(i)
...
def maximum_number() -> dict:
    ...
    for i in o_range:
        ...
        # maximum = set()
        # for maxi in maxi_lst:
        #     s_maxi = str(maxi)
        #     f_maxi = float(s_maxi[:3])
        #     maximum.add(f_maxi)
        # max_dict[i] = list(maximum)
